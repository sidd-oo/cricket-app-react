...
const PaginationButton = ({ type, selectPageHandler, pageNo }) => {
  return (
    <button
      ...
      onClick={() => selectPageHandler(type==="Next" ? (pageNo + 1) : (pageNo - 1))}
    >
...
const Pagination = ({ pageNo, setPageNo, playersList }) => {
  const selectPageHandler = (selectedPage) => {
    if (
      selectedPage >= 1 &&
      selectedPage <= Math.ceil(playersList.length / paginationPerPageSize) &&
      selectedPage !== pageNo
    ) {
  ...
  return (
    <div className="mt-1 flex items-center justify-end space-x-2">
      <PaginationButton
        ...
        pageNo={pageNo}
      />
      {[...Array(Math.ceil(playersList.length / paginationPerPageSize))].map((_, i) => {
        return (
          <span
            key={i}
            className={
